...
import pytest
from recordsQL import WITH, SELECT, cols, col, text, num
...
class TestWithBasic:
    ...
    def test_with_cte_and_where(self):
        ...
        with_query = WITH(inner_query.AS("adults")).SELECT(name).FROM("adults").WHERE(
            city == "New York"
        )
        sql, params = with_query.placeholder_pair()
        assert 'WITH' in sql
...
class TestWithAdvanced:
    ...
    def test_with_order_by_limit_offset(self):
        ...
        inner_query = SELECT(name, created_at).FROM("users").ORDER_BY(
            created_at, "DESC"
        ).LIMIT(100)
        
        with_query = WITH(inner_query.AS("recent_users")).SELECT(name).FROM(
            "recent_users"
        ).LIMIT(10).OFFSET(5)
    ...
    def test_with_and_group_by(self):
        ...
        with_query = WITH(inner_query.AS("high_earners")).SELECT(
            "department"
        ).FROM("high_earners").GROUP_BY("department")
    ...
    def test_with_multiple_conditions(self):
        ...
        inner_query = SELECT("name", "age", "status").FROM("users").WHERE(
            (age > 18) & (status == "active") & (verified == True)
        )
        
        with_query = WITH(inner_query.AS("active_users")).SELECT("name", "age").FROM(
            "active_users"
        ).WHERE(age < 65)
    ...
    def test_with_and_having(self):
        ...
        with_query = WITH(inner_query.AS("dept_data")).SELECT(
            "department"
        ).FROM("dept_data").GROUP_BY("department").HAVING(count > 10)
    ...
    def test_with_numeric_operations(self):
        ...
        inner_query = SELECT(price, discount, quantity).FROM("products").WHERE(
            (price - discount) > 50
        )
        
        with_query = WITH(inner_query.AS("discounted_products")).SELECT(
            price, discount
        ).FROM("discounted_products").WHERE(quantity > 0)
    ...
    def test_with_string_comparison(self):
        ...
        inner_query = SELECT(name, category).FROM("products").WHERE(
            category == "Electronics"
        )
        
        with_query = WITH(inner_query.AS("electronics")).SELECT(name).FROM(
            "electronics"
