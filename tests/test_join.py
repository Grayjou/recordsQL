...
class TestJoinBasic:
    ...
    def test_inner_join(self):
        ...
        query = SELECT().FROM("users").INNER_JOIN(
            table_name="orders",
            on=(user_id == col("order_user_id"))
        )
    ...
    def test_left_join(self):
        """Test LEFT JOIN"""
        user_id = col("user_id")
        query = SELECT().FROM("users").LEFT_JOIN(
            table_name="orders",
            on=(user_id == col("order_user_id"))
        )
        sql, params = query.placeholder_pair()
    ...
    def test_right_join(self):
        """Test RIGHT JOIN"""
        user_id = col("user_id")
        query = SELECT().FROM("users").RIGHT_JOIN(
            table_name="orders",
            on=(user_id == col("order_user_id"))
        )
        sql, params = query.placeholder_pair()
    ...
    def test_full_join(self):
        """Test FULL JOIN"""
        user_id = col("user_id")
        query = SELECT().FROM("users").FULL_JOIN(
            table_name="orders",
            on=(user_id == col("order_user_id"))
        )
        sql, params = query.placeholder_pair()
...
class TestJoinAdvanced:
    ...
    def test_multiple_joins(self):
        ...
        query = (
            ...
            .LEFT_JOIN(
                table_name="payments",
                on=(order_id == col("payment_order_id"))
            )
        )
        sql, params = query.placeholder_pair()
        assert 'INNER JOIN' in sql
    ...
    def test_join_with_numeric_condition(self):
        ...
        query = SELECT().FROM("products").INNER_JOIN(
            table_name="prices",
    ...
    def test_with_query_and_joins(self):
        ...
        with_query = WITH(select_query.AS("adult_users")).SELECT(
            name, age, email
        ).FROM("adult_users")
    ...
    def test_three_table_join(self):
        ...
        query = (
            ...
            .INNER_JOIN(
                table_name="products",
                on=(product_id == col("order_product_id"))
            )
        )
        sql, params = query.placeholder_pair()
