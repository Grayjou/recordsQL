...
def update_query_for(
    ...
    **kwargs,
    ...
    set_clauses_in_self = {
        key: value
        for key, value in initial_set_clauses.items()
        if key in table.column_dict
    }

    if set_clauses is not None:
        if not isinstance(set_clauses, dict):
            raise TypeError("set_clauses must be a dictionary.")
        set_clauses_in_self.update(
            {
                key: value
                for key, value in set_clauses.items()
                if key in table.column_dict
            }
        )
