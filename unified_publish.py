...
def build_package(package_path: Path) -> bool:
    ...
    try:
        result = subprocess.run([sys.executable, "-m", "build"], cwd=str(package_path), capture_output=True, text=True)
...
def upload_package(package_path: Path, token: str, target: str, dry_run: bool = False) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        result = subprocess.run(cmd, cwd=str(package_path), capture_output=True, text=True)
